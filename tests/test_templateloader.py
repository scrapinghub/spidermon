...
def test_get_template_with_absolute_path(tmpdir: os.PathLike[str]) -> None:
...
def test_fail_get_absolute_template_that_does_not_exist(
    tmpdir: os.PathLike[str],
) -> None:
    template_loader = TemplateLoader()
...
def test_fail_get_template_that_does_not_exist(tmpdir: os.PathLike[str]) -> None:
    template_loader = TemplateLoader()
    with pytest.raises(jinja2.exceptions.TemplateNotFound):

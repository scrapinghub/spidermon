...
@pytest.fixture
def suites() -> list[str | type[MonitorSuite]]:
    return ["tests.fixtures.suites.Suite01"]


def test_spider_opened_suites_should_run(
    get_crawler: Callable[..., Crawler], suites: list[str | type[MonitorSuite]]
) -> None:
    ...
    with mock.patch.object(spidermon.spider_opened_suites[0], "run") as run:
        spidermon.spider_opened(crawler.spider)
    assert spidermon.spider_opened_suites[0].__class__.__name__ == "Suite01"
    run.assert_called_once_with(mock.ANY)


def test_spider_closed_suites_should_run(
    get_crawler: Callable[..., Crawler], suites: list[str | type[MonitorSuite]]
) -> None:
    ...
    with mock.patch.object(spidermon.spider_closed_suites[0], "run") as run:
        spidermon.spider_opened(crawler.spider)
...
def test_engine_stopped_suites_should_run(
    get_crawler: Callable[..., Crawler], suites: list[str | type[MonitorSuite]]
) -> None:
    ...
    with mock.patch.object(spidermon.engine_stopped_suites[0], "run") as run:
        spidermon.engine_stopped()
    assert spidermon.engine_stopped_suites[0].__class__.__name__ == "Suite01"
    run.assert_called_once_with(mock.ANY)


def test_spider_opened_suites_should_run_from_signal(
    get_crawler: Callable[..., Crawler], suites: list[str | type[MonitorSuite]]
) -> None:
    ...
    with mock.patch.object(spidermon.spider_opened_suites[0], "run") as run:
        crawler.signals.send_catch_log(
            signal=signals.spider_opened, spider=crawler.spider
        )
    run.assert_called_once_with(mock.ANY)


def test_spider_closed_suites_should_run_from_signal(
    get_crawler: Callable[..., Crawler], suites: list[str | type[MonitorSuite]]
) -> None:
    ...
    with mock.patch.object(spidermon.spider_closed_suites[0], "run") as run:
        crawler.signals.send_catch_log(
            signal=signals.spider_closed, spider=crawler.spider
        )
    run.assert_called_once_with(mock.ANY)


def test_engine_stopped_suites_should_run_from_signal(
    get_crawler: Callable[..., Crawler], suites: list[str | type[MonitorSuite]]
) -> None:
    ...
    with mock.patch.object(spidermon.engine_stopped_suites[0], "run") as run:
        crawler.signals.send_catch_log(
            signal=signals.engine_stopped, spider=crawler.spider
        )
    run.assert_called_once_with(mock.ANY)

...
SPIDERMON_EXTENSION = {"spidermon.contrib.scrapy.extensions.Spidermon": 500}
EXTENSIONS_STRING = " = {}"
EXTENSIONS_UPDATE_STRING = ".update({})"
...
def enable_spidermon():
    ...
    extensions_to_write = EXTENSIONS_STRING
    if extensions:
        extensions_to_write = EXTENSIONS_UPDATE_STRING
    extensions_to_write = extensions_to_write.format(str(SPIDERMON_EXTENSION))

    project_name = get_project_settings().get("BOT_NAME")
    formatted_settings = "\n".join(MONITOR_SETTINGS).format(
        extensions_to_write, project_name
    )

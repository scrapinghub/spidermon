...
def test_item_count_monitor_extracted_more_than_expected(
        make_data, item_count_suite):
...
def test_item_count_monitor_extracted_less_than_expected(
        make_data, item_count_suite):
    ...
    runner.run(item_count_suite, **data)
    assert('Extracted 50 items, the expected minimum is 100' in
...
def test_unwanted_httpcodes_should_fail(make_data):
    ...
    runner.run(suite, **data)
    assert('Found 11 Responses with status code=500'
